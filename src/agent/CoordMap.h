...
public:
  CoordinateMap(CellLayer * p_layer) 
    : CellGroup(p_layer), _p_layer(p_layer),
      ...
      _endX(_oriX + _extentX - 1), _endY(_oriY + _extentY - 1),
  ...
  bool addCellAt(int, const repast::Point<int> &); 
  ...
  void setBorder(const std::string &, CoordinateMap *);
...
void CoordinateMap<N>::setBorder(
    const std::string & direction, CoordinateMap * p_border)
{
  if (direction == "N") 
  { 
...
template <size_t N>
bool CoordinateMap<N>::addCellAt(int cellState, const repast::Point<int> & pt) 
{ 
