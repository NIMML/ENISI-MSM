...
std::string Diffuser::classname()
{
  return "Diffuser";
}
void Diffuser::diffuse()
{
  p_impl->diffuse();
}

double Diffuser::getDiffusionConst()
{
  return p_impl->getDiffusionConst();
}
void Diffuser::setDiffusionConst(double d)
{
  p_impl->setDiffusionConst(d);
}

double Diffuser::getEvaporationConst()
{
  return p_impl->getEvaporationConst();
}
void Diffuser::setEvaporationConst(double e)
{
  p_impl->setEvaporationConst(e);
}
...
void Diffuser::setGridPointsToCalculate(
    const std::vector<repast::Point<int> > & p)
{
...
void Diffuser::setValuesAtCoords(std::vector<double> & values,
                                 std::vector<repast::Point<int> > coords)
{
  return p_impl->setValuesAtCoords(values, coords);
}

std::vector<double> Diffuser::getAssignedGridValues()
{
  return p_impl->getAssignedGridValues();
}

double ** Diffuser::getGrid()
{
  return p_impl->getGrid();
}
void Diffuser::setGrid(double ** newGrid)
{
  p_impl->setGrid(newGrid);
}
...
DiffuserImpl::DiffuserImpl(const repast::GridDimensions dims, double e,
                           double d, bool t) :
    maxValue(DEFAULT_MAX), _minValue(DEFAULT_MIN), evaporationConst(e), diffusionConst(
        d), toroidal(t), _dimensions(dims), _width(
        _dimensions.extents().getX() + 1), _height(
        _dimensions.extents().getY() + 1), _borders(_dimensions)
{
...
const repast::GridDimensions & DiffuserImpl::getDimensions() const
{
  return _dimensions;
}
 ...
 */
std::vector<repast::Point<int> > DiffuserImpl::getGridCoordsAssignedToRank(
    int rank)
{
...
void DiffuserImpl::setValuesAtCoords(std::vector<double> & values,
                                     std::vector<repast::Point<int> > coords)
{
...
std::vector<double> DiffuserImpl::getAssignedGridValues()
{
  ...
  std::vector<repast::Point<int> >::const_iterator it =
      _gridPointsToDiffuse.begin();
...
repast::Point<int> DiffuserImpl::vectorToPointCoordinates(
    const std::vector<int>& vCoords)
{
...
double DiffuserImpl::compute2dValueAtCoord(int x, int y)
{
  ...
  double weightedAvg = ((uE + uN + uW + uS) * 4 + (uNE + uNW + uSW + uSE))
      / 20.0;
