...
DendriticsGroup::DendriticsGroup(
    const unsigned int & dendriticsCount, repast::SharedContext<ENISIAgent> * ctx)
  : ENISIAgent(ctx)
{
  for (unsigned int i = 0; i < dendriticsCount; i++) 
  {
...
void DendriticsGroup::act(State state, const repast::Point<int> & loc)
{
  ...
  if (newState == EFFECTOR) 
  {
    ...
    cytoMap["IL6"]->set(70, loc);
    cytoMap["IL12"]->set(70, loc);
  }
  else if (newState == TOLEROGENIC) 
  {
    Cytokines::CytoMap & cytoMap = Cytokines::instance().map();
    cytoMap["TGFb"]->set(70, loc);
  }
