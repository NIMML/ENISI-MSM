...
BacteriaGroup::BacteriaGroup(
    const unsigned int & bacteriaCount, repast::SharedContext<ENISIAgent> * ctx)
  : ENISIAgent(ctx)
{
  init(bacteriaCount, ctx);
}

void BacteriaGroup::init(
    const unsigned int & bacteriaCount, repast::SharedContext<ENISIAgent> * ctx)
{
  for (unsigned int i = 0; i < bacteriaCount; i++) 
  {
