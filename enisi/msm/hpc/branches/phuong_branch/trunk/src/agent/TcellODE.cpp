...
TcellODE::TcellODE() : DEBUG(false)
{
  ...
  std::string modelFileName = "./modelfiles/MSM_CD4.cps"; /* small model */
  //std::string modelFileName = "./modelfiles/CD4.cps";     [> big model <]

  // Duplicate file check because loadModel() won't fail with bad paths
  ...
  try
  {
    ...
    dataModel->loadModel(modelFileName, NULL);
  }
  catch (...)
  {
    std::cerr << "Error while loading the model from file named \"" 
